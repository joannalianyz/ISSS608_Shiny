...
We are aligned with the sentiments of the two papers mentioned above and they have inspired our approach to this project. The motivation and objectives of our project are elaborated on in the next section.
...
2. First module - What are the employees spending on? We will conduct EDA and inferential analysis on the credit card transaction data. On the side panel, we will have 2 tabs - 1 for EDA (a histogram and a data table) and 1 (a violin plot) for inferential analysis.
...
![](IMG/2.PNG)
![](IMG/3.PNG)
    Depending on our selection of the various filters, the visualisations in the main panel will be automatically filtered to represent the data corresponding to the filters. For the purpose of our project, we have excluded anomalous transactions when constucting the violin plots. The full range of transactions can nontheless still be viewed through the histogram and the data table.
